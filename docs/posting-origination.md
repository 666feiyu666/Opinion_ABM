...
where $\pi_0\in(0,1)$ is an ordinary agent's origination probability in round 1, $\lambda\geq0$ is the common rate of interest decay, $\beta_L\geq0$ is the opinion-leader advantage, and $\sigma(x)=1/(1+e^{-x})$. Leader status therefore multiplies the odds of origination by $e^{\beta_L}$ while both roles experience the same temporal decline.
